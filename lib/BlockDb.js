...
BlockDb.prototype.getBlockForTx = function(txId, cb) {
  bitcoindjs.getBlockByTxid(txId, cb);
};
...
BlockDb.prototype.getPrev = function(hash, cb) {
  bitcoindjs.getBlock(hash, function(err, block) {
    if (err) return cb(err);
    bitcoindjs.getBlock(block.previousblockhash, function(err, block) {
      if (err) return cb(err);
...
BlockDb.prototype.getNext = function(hash, cb) {
  bitcoindjs.getBlock(hash, function(err, block) {
    if (err) return cb(err);
    bitcoindjs.getBlock(block.nextblockhash, function(err, block) {
      if (err) return cb(err);
      return callback(null, block);
    });
  });
